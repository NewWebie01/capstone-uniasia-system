...
export default function InventoryPage() {
  ...
  const computedWeight = useMemo(() => {
    ...
    // Kung Kg, ang weight ay yung mismong quantity (assuming 1kg per qty)
    if (unit === 'Kg') return qty;

    const weightPerPiece = Number(newItem.weight_per_piece_kg) || 0;

    // Dito ang logic: Kung walang value sa pieces_per_unit, default to 1 (Piece/Box/Pack)
    // Pero kung Dozen, automatic 12.
    const piecesPerUnit =
      Number(newItem.pieces_per_unit) || (unit === 'Dozen' ? 12 : 1);

    return weightPerPiece * piecesPerUnit * qty;
  }, [
    newItem.unit,
            ...
            <tbody>
              {paged.map((item) => (
                ...
                >
                  ...
                  <td className={cellNowrap}>
                    {(() => {
                      // Kinukuha natin ang value diretso sa item (wala sa DB, "on-the-fly" calculation)
                      const wpp = Number(item.weight_per_piece_kg) || 0;
                      const ppu =
                        Number(item.pieces_per_unit) ||
                        (item.unit === 'Dozen' ? 12 : 1);
                      const qty = Number(item.quantity) || 0;
                      const total = wpp * ppu * qty;

                      // I-display ang calculation (kahit 0, lalabas siya)
                      return total > 0
                  ...
                  <td className={cellNowrap}>
                    {(() => {
                      ...
                      const isExpiringSoon =
                        expDate && expDate <= thirtyDaysFromNow && !isExpired;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <Row label="Item Size">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="flex-1 min-w-0">
                        {/* (Dito mo ilalagay ang Popover code mo para sa Size) */}
                        {/* Siguraduhin na tanggalin ang condition na 'isPipe' sa taas */}
                      </div>
                    </div>
                  </Row>
