...
/** Normalize to a Date.
 * - If the string already has a timezone (ends with Z or ±HH:MM), use as-is.
 * - If the string is "naive" (no TZ), parse as UTC then SHIFT by −8 hours
 *   so the final render in Asia/Manila matches your intended local time.
 */
function toPHDate(v?: string | number | Date | null): Date | null {
  if (v === null || v === undefined) return null;

  if (typeof v === "string") {
    const s = v.trim();
    // if the string has no TZ (naive), treat it as UTC by appending Z
...
/** Force-render as Asia/Manila local date+time */
function formatPHDateTimeTZ(v?: string | number | Date | null): string {
  const d = toPHDate(v);
  if (!d) return "—";
...
/** Force-render as Asia/Manila date (no time) */
function formatPHDateOnlyTZ(v?: string | number | Date | null): string {
  const d = toPHDate(v);
  if (!d) return "—";
  return new Intl.DateTimeFormat("en-PH", {
    year: "numeric",
    ...
    <div className="min-h-[calc(100vh-80px)]">
      <div className="mx-auto w-full max-w-6xl px-6 py-6">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Cheques appear here after an admin marks them as <b>Received</b> or{" "}
          <b>Rejected</b>.
        </p>
                  ...
                  return (
                    ...
                    >
                      <td className="py-2.5 px-3 whitespace-nowrap">
                        {formatPHDateTimeTZ(p.received_at || p.created_at)}
                      </td>
                      ...
                      <td className="py-2.5 px-3 whitespace-nowrap">
                        {p.cheque_date
                          ? formatPHDateOnlyTZ(p.cheque_date)
                          : "—"}
                      </td>
                      <td className="py-2.5 px-3 whitespace-nowrap">
                        {p.image_url ? (
        ...
        <div className="mt-4 flex items-start gap-2 text-sm text-gray-600">
          ...
          <span>
            This page shows cheques after admin review — both <b>Received</b>{" "}
            and <b>Rejected</b>.
          </span>
        </div>
      </div>
