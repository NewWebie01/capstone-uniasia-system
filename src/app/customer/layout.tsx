...
export default function CustomerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (user) setUserName((user.user_metadata as any)?.name || user.email || "Guest");
    };
    getUser();
  }, []);
    ...
    <CartProvider>
      ...
      >
            ...
            <div className="max-w-7xl mx-auto relative">
              <div className="absolute top-0 right-6 flex items-center gap-3">
                ...
                <div className="greeting-card bg-white rounded-xl px-4 py-2 text-gray-700 shadow-md select-none cursor-default hover:shadow-lg">
                  ...
                  <span className="text-[#ffba20] font-bold">{userName || "Guest"}</span>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
              <div className="px-6 py-5 flex items-start gap-3 border-b">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Desktop Only</h2>
                  <p className="mt-1 text-sm text-gray-600">
                    Our Customer Portal is currently optimized for <span className="font-semibold">desktop or laptop</span> screens.
                  </p>
                </div>
              </div>
              ...
              <div className="px-6 py-4 bg-gray-50 border-t text-xs text-gray-500">Need help? Contact UNIASIA support.</div>
            </div>
          </div>
        )}
        ...
        <style jsx global>{`
          ...
          .greeting-card:hover { animation: greeting-wobble 0.5s ease-in-out both; }
        `}</style>
      </div>
    </CartProvider>
  );
